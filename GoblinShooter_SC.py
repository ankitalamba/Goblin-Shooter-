...
win = pygame.display.set_mode((500,480)) #rendering the game screen with dimensions 500,500
pygame.display.set_caption("Goblin Shooter") #Title of the game
...
class enemy(object):
    ...
    def __init__(self,x,y,width,height,end):
        ...
        self.width = width
        self.height = height
        self.path =[x, end] #path of the enemy
        self.walkCount = 0
        self.vel = 3
    ...
    def draw(self,win):
        ...
        if self.visible :
            ...
            pygame.draw.rect(win,(20,160,0),(self.hitbox[0],self.hitbox[1] - 20, 50 - (5 * (10 - self.health)),10))
            self.hitbox = (self.x + 17, self.y + 2, 31,57)
        #pygame.draw.rect(win,(255,0,0),self.hitbox,2)
    def move(self):
        if self.vel > 0 : #if we're moving right
            if self.x < self.path[1] + self.vel : #basically not on the rightest part of the screen
                self.x += self.vel
            else :
                self.vel = self.vel * -1
                self.x += self.vel
            ...
            if self.x > self.path[0] - self.vel :
                ...
                self.x += self.vel
                self.walkCount = 0
    def hit(self) : #check the collision between the bullets and the goblin
            hitSound.play()
            if self.health > 0:
                self.health -= 1
            else:
...
class player(object):
    ...
    def draw(self,win):
        if self.walkCount + 1 >= 27: #character will remain within the screen
            self.walkCount = 0
        if not(self.standing):
            if self.left :
                win.blit(walkLeft[self.walkCount//3],(self.x,self.y))
                self.walkCount += 1#we integer divide walkCount by 3 to ensure each image is shown 3 times every animation
            elif self.right :
    ...
    def hit(self):
        ...
        self.y = 410
        self.walkCount = 0
        f1 = pygame.font.SysFont('comicsans',50)
        text = f1.render('-5',1,(255,0,0))
        win.blit(text,(250 - (text.get_width()/2),200))
        pygame.display.update()
        i = 0
        while i < 200 :
            pygame.time.delay(10)#0.01 milliseconds
            i += 1
...
while run:
    ...
    for bullet in bullets:
        ...
        if bullet.x < 500 and bullet.x > 0 :
            bullet.x += bullet.vel #moves the bullet with it's velocity
        else:
            bullets.pop(bullets.index(bullet)) #this will remove the bullet if its off the screen
 
    keys = pygame.key.get_pressed() #returning a list
    ...
    if not(man.isJump): 
        ...
        if man.jumpCount >= -10: 
            man.y -= (man.jumpCount * abs(man.jumpCount)) * 0.5 #jumpcount(10 to -10)
            man.jumpCount -= 1
        else: 
            man.jumpCount = 10
            man.isJump = False   
